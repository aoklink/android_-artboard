package com.linkfeeling.android.art.board.third.adapterdelegate;
...
import com.linkfeeling.android.art.board.third.adapterdelegate.adapter.EmptyAdapterDelegate;
...
public final class AdapterDelegatesManager<T> {
        ...
        if (items instanceof List && ((List) items).size() > position){
            Log.e("AdapterDelegatesManager", " invalid viewType for " + position + " item" + ((List)items).get(position));
        }
